...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        ...
        "Marketing": ["AC Performance", "Referrals","HubSpot Deal Score tracker","Marketing Lead Performance & Requirement","Kids detail", "Deal Detail", "Sales Intern Funnel", "Master analysis", "Referral Tracking", "Talk Time", "referral_Sibling"],
    }
...
# =====================
# Marketing ▸ AC Performance (Academy Consolidated Performance)
# =====================
try:
    if "AC Performance" not in MASTER_SECTIONS.get("Marketing", []):
        MASTER_SECTIONS["Marketing"] = ["AC Performance"] + MASTER_SECTIONS.get("Marketing", [])
except Exception:
    pass

def _acp_col_guess(_df, candidates):
    ...
    for c in candidates:
        for col in _df.columns:
            if str(c).lower().strip() in str(col).lower():
                return col
    return None

def _acp_parse_date_series(s):
...
def _acp_date_controls(key="acp"):
    import streamlit as st
    tz = "Asia/Kolkata"
    ...
    if preset == "Today":
        ...
        c1, c2 = st.columns(2)
        with c1:
            start = st.date_input("Start", value=first, key=f"{key}_start")
        with c2:
            end = st.date_input("End", value=today, key=f"{key}_end")
...
def _acp_render_card(ac_name, dfv, start_d, end_d, peers_df, cfg):
    import streamlit as st
    deals = len(dfv)
    enroll = int(dfv["_pay_d"].notna().sum())
    ts = int(((dfv["_rs_d"].notna()) | (dfv["_rs_d"].isna() & dfv["_fs_d"].notna())).sum())
    td = int(dfv["_done_d"].notna().sum())
    lead_to_pay = (0.0 if deals == 0 else round(100.0 * enroll / deals, 1))
    lead_to_enr = lead_to_pay
    ref_mask = dfv[cfg["ref_col"]].notna() if cfg["ref_col"] else pd.Series(False, index=dfv.index)
    ref_deals = int(ref_mask.sum())
    ref_enroll = int((ref_mask & dfv["_pay_d"].notna()).sum())
    ref_conv = (0.0 if ref_deals == 0 else round(100.0 * ref_enroll / ref_deals, 1))
    seven_days_ago = pd.to_datetime(end_d) - pd.Timedelta(days=7)
    last_act_col = cfg["last_act_col"]
    if last_act_col:
        last_act = pd.to_datetime(dfv[last_act_col], errors="coerce", dayfirst=True)
        not_conn_ac = int((last_act < seven_days_ago).sum())
        not_conn_owner = int((last_act < seven_days_ago).sum())
    else:
        not_conn_ac = None; not_conn_owner = None
    src_col = cfg["src_col"]
    split_series = dfv[src_col].fillna("Unspecified") if src_col else pd.Series([], dtype=object)
    src_counts = split_series.value_counts()
    src_total = int(src_counts.sum()) if len(src_counts) else 0
    src_pct = (src_counts / max(1, src_total) * 100.0).round(1)
    peers = peers_df.copy()
    peers["deals"] = peers["df"].apply(lambda x: len(x))
    peers["enroll"] = peers["df"].apply(lambda x: int(x["_pay_d"].notna().sum()))
    peers["ts"] = peers["df"].apply(lambda x: int(((x["_rs_d"].notna()) | (x["_rs_d"].isna() & x["_fs_d"].notna())).sum()))
    peers["td"] = peers["df"].apply(lambda x: int(x["_done_d"].notna().sum()))
    peers["lead_to_pay"] = peers.apply(lambda r: (0.0 if r["deals"] == 0 else 100.0 * r["enroll"] / r["deals"]), axis=1)
    peers["lead_to_enr"] = peers["lead_to_pay"]
    if cfg["ref_col"]:
        peers["ref_deals"] = peers["df"].apply(lambda x: int(x[cfg["ref_col"]].notna().sum()))
        peers["ref_enroll"] = peers["df"].apply(lambda x: int((x[cfg['ref_col']].notna()) & (x['_pay_d'].notna())).sum())
        peers["ref_conv"] = peers.apply(lambda r: (0.0 if r["ref_deals"] == 0 else 100.0 * r["ref_enroll"] / r["ref_deals"]), axis=1)
    else:
        peers["ref_conv"] = 0.0
    avg_deals = peers["deals"].mean() if len(peers) else 0.0
    avg_enroll = peers["enroll"].mean() if len(peers) else 0.0
    avg_ts = peers["ts"].mean() if len(peers) else 0.0
    avg_td = peers["td"].mean() if len(peers) else 0.0
    avg_l2p = peers["lead_to_pay"].mean() if len(peers) else 0.0
    avg_ref_conv = peers["ref_conv"].mean() if len(peers) else 0.0
    rank_enroll = int((peers["enroll"].rank(method="min", ascending=False) ).to_dict().get(ac_name, 0)) if ac_name in peers.index else 0
    rank_ref = int((peers["ref_conv"].rank(method="min", ascending=False) ).to_dict().get(ac_name, 0)) if ac_name in peers.index else 0
    st.markdown("---")
    ctitle = st.container()
    with ctitle:
        st.markdown(f"### 🎓 {ac_name}")
        st.caption(f"Report as on {pd.Timestamp(end_d).strftime('%d %b %Y')}")
    def _arrow(val, avg):
        if val > avg: return "▲"
        if val < avg: return "▼"
        return "•"
    k1,k2,k3,k4,k5,k6 = st.columns(6)
    k1.metric("Deals", f"{deals}", delta=_arrow(deals, avg_deals))
    k2.metric("Enrollments", f"{enroll}", delta=_arrow(enroll, avg_enroll))
    k3.metric("Trials Scheduled", f"{ts}", delta=_arrow(ts, avg_ts))
    k4.metric("Trials Done", f"{td}", delta=_arrow(td, avg_td))
    k5.metric("Lead→Pay %", f"{lead_to_pay:.1f}%", delta=_arrow(lead_to_pay, avg_l2p))
    k6.metric("Lead→Enrollment %", f"{lead_to_enr:.1f}%", delta=_arrow(lead_to_enr, avg_l2p))
    r1, r2, r3, r4 = st.columns(4)
    r1.metric("Referral Conversion %", f"{ref_conv:.1f}%")
    r2.metric("Referral Enrollments", f"{ref_enroll}")
    r3.metric("Not connected ≥7d (AC)", "—" if not_conn_ac is None else f"{not_conn_ac}")
    r4.metric("Not connected ≥7d (Owner)", "—" if not_conn_owner is None else f"{not_conn_owner}")
    try:
        import altair as alt
        if src_total > 0:
            plot_df = src_pct.reset_index()
            plot_df.columns = ["Deal Source","Percent"]
            plot_df["row"] = "split"
            chart = alt.Chart(plot_df).mark_bar().encode(
                x=alt.X("Percent:Q", stack="normalize"),
                y=alt.Y("row:N", axis=None),
                color="Deal Source:N",
                tooltip=["Deal Source","Percent"]
            ).properties(height=40)
            st.markdown("**Deal Source split**")
            st.altair_chart(chart, use_container_width=True)
    except Exception as _e:
        st.caption(f"(Deal Source split unavailable: {_e})")

def _render_marketing_ac_performance(df_f: pd.DataFrame):
    import streamlit as st
    st.subheader("AC Performance (Academy Consolidated)")
    df = df_f.copy()
    stage_col = _acp_col_guess(df, ["Deal Stage","Stage"])
    if stage_col is not None:
        df = df[~df[stage_col].astype(str).str.contains("invalid", case=False, na=False)].copy()
    ac_col = _acp_col_guess(df, ["Student/Academic Counsellor","Academy Consolidated","Academy Consolidated Name"])
    create_col = _acp_col_guess(df, ["Create Date","Deal Create Date","Created Date"])
    pay_col = _acp_col_guess(df, ["Payment Received Date","Payment Received Date ","Enrollment Date"])
    fs_col = _acp_col_guess(df, ["First Calibration Scheduled Date","First Calibration Booking Date","Calibration Scheduled Date"])
    rs_col = _acp_col_guess(df, ["Calibration Rescheduled Date","Trial Reschedule 1","Trial Reschedule 2"])
    done_col = _acp_col_guess(df, ["Calibration Done Date"])
    src_col = _acp_col_guess(df, ["JetLearn Deal Source","Deal Source","Source"])
    last_act_col = _acp_col_guess(df, ["Last Activity Date","Last Connected (AC)","AC Last Connected","Last Spoke Date (AC)","Last Contacted (AC)"])
    ref_col = _acp_col_guess(df, ["Referral Intent Source","Referral Intent","Referral Source","Referral/Sibling","Referral"])
    if ac_col is None or create_col is None:
        st.error("Required columns missing: Academic Counsellor / Create Date")
        return
    df["_create_d"] = _acp_parse_date_series(df[create_col])
    df["_pay_d"] = _acp_parse_date_series(df[pay_col]) if pay_col is not None else pd.NaT
    df["_fs_d"] = _acp_parse_date_series(df[fs_col]) if fs_col is not None else pd.NaT
    df["_rs_d"] = _acp_parse_date_series(df[rs_col]) if rs_col is not None else pd.NaT
    df["_done_d"] = _acp_parse_date_series(df[done_col]) if done_col is not None else pd.NaT
    mode, start, end = _acp_date_controls("acperf")
    start_d = pd.to_datetime(start).date(); end_d = pd.to_datetime(end).date()
    if mode == "MTD":
        base = df["_create_d"].between(start_d, end_d, inclusive="both")
        view = df.loc[base].copy()
    else:
        view = df.copy()
    ac_list = sorted(list(view[ac_col].dropna().unique()))
    all_toggle = st.checkbox("All", value=True, key="acperf_all_toggle")
    if all_toggle:
        selected = ac_list
    else:
        selected = st.multiselect("Select Academy Consolidated", options=ac_list, default=ac_list[:1], key="acperf_ac_multi")
    peers = []
    for name in selected:
        if mode == "MTD":
            sub = view[view[ac_col] == name].copy()
        else:
            sub_all = df[df[ac_col] == name].copy()
            sub = sub_all[
                (sub_all["_create_d"].between(start_d, end_d, inclusive="both")) |
                (sub_all["_pay_d"].between(start_d, end_d, inclusive="both")) |
                (sub_all["_fs_d"].between(start_d, end_d, inclusive="both")) |
                (sub_all["_rs_d"].between(start_d, end_d, inclusive="both")) |
                (sub_all["_done_d"].between(start_d, end_d, inclusive="both"))
            ].copy()
        peers.append((name, sub))
    peers_df = pd.DataFrame({"ac": [n for n,_ in peers], "df": [d for _,d in peers]}).set_index("ac")
    st.caption(f"Mode: {mode} • Window: {start_d} → {end_d}")
    cfg = {"src_col": src_col, "last_act_col": last_act_col, "ref_col": ref_col}
    if all_toggle:
        if mode == "MTD":
            df_all = view.copy()
        else:
            df_all = df.copy()
        _acp_render_card("All (Vertical)", df_all, start_d, end_d, peers_df, cfg)
    for name in selected:
        if mode == "MTD":
            sub = view[view[ac_col] == name].copy()
        else:
        ...
        _acp_render_card(name, sub, start_d, end_d, peers_df, cfg)
...
try:
    ...
    import streamlit as _st
    _st.error(f"AC Performance error: {_e}")
