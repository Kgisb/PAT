...
# Ensure the new pill appears under Funnel & Movement
try:
...
def _trial_sched_flag_row(fs_date, rs_date):
    # If both exist -> use rescheduled; else use first scheduled; return 1/0 plus chosen date
    fs = pd.notna(fs_date)
    rs = pd.notna(rs_date)
    if rs:
        return 1, rs_date
...
def _render_funnel_jl_source_analysis(df_f: pd.DataFrame):
    ...
    cal_done_col = _col_guess(df, ["Calibration Done Date"])  # Trial Done = Calibration Done Date only
    ...
    # Normalize dates and helper columns
    ...
    # Derive unified scheduled event date (prefer rescheduled)
    ...
    # Build masks per mode
    if mode == "MTD":
        base = df["_create_d"].between(start_d, end_d, inclusive="both")
        dfv = df.loc[base].copy()
        # metric flags
        deals_mask = pd.Series(True, index=dfv.index)
        enrl_mask = dfv["_pay_d"].notna()  # count enrolments among deals created in window (regardless of pay date)
        sched_mask = dfv["_sched_flag"] == 1  # count scheduled among deals created in window
        done_mask = dfv["_done_d"].notna()   # count done among deals created in window
    else:  # Cohort
        dfv = df.copy()
        deals_mask = dfv["_create_d"].between(start_d, end_d, inclusive="both")  # deals are by create date
    ...
    # Aggregate by JetLearn Deal Source
    ...
    try:
        st.download_button(
            "Download JL Source-wise table (CSV)",
...
# Dispatcher for Funnel & Movement → JL Source-wise Analysis
try:
