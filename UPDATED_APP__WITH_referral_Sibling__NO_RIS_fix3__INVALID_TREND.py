...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        ...
        "Marketing": ["Invalid deal trend", "Referrals","HubSpot Deal Score tracker","Marketing Lead Performance & Requirement","Kids detail", "Deal Detail", "Sales Intern Funnel", "Master analysis", "Referral Tracking", "Talk Time", "referral_Sibling"],
    }
...
# =====================
# Marketing ▸ Invalid deal trend
# =====================
try:
    if "Invalid deal trend" not in MASTER_SECTIONS.get("Marketing", []):
        MASTER_SECTIONS["Marketing"] = ["Invalid deal trend"] + MASTER_SECTIONS.get("Marketing", [])
except Exception:
    pass

def _mdt_parse_date_series(s):
...
def _mdt_col_guess(_df, candidates):
...
def _mdt_date_controls(key="inv"):
    import streamlit as st
    tz = "Asia/Kolkata"
...
def _floor_period(series, gran):
    idx = pd.to_datetime(series, errors="coerce")
    if gran.startswith("Day"):
...
def _render_marketing_invalid_deal_trend(df_f: pd.DataFrame):
    import streamlit as st
    st.subheader("Invalid deal trend")
    df = df_f.copy()
    stage_col = _mdt_col_guess(df, ["Deal Stage","Stage","deal stage"])
    create_col = _mdt_col_guess(df, ["Create Date","Deal Create Date","Created Date"])
    if stage_col is None or create_col is None:
        st.error("Required columns not found: Deal Stage / Create Date")
        return
    df["_is_invalid"] = df[stage_col].astype(str).str.contains("invalid", case=False, na=False)
    if not df["_is_invalid"].any():
        st.info("No rows with 'invalid' found in Deal Stage.")
        return
    df["_create_d"] = _mdt_parse_date_series(df[create_col])
    inv = df.loc[df["_is_invalid"]].copy()
    inv["_invalid_type"] = df[stage_col].astype(str)
    mode, start, end, gran, split = _mdt_date_controls("invdeal")
    start_d = pd.to_datetime(start).date(); end_d = pd.to_datetime(end).date()
    if mode == "MTD":
        base = inv["_create_d"].between(start_d, end_d, inclusive="both")
        view = inv.loc[base].copy()
        view["_ev_d"] = view["_create_d"]
    else:
        view = inv.copy()
        view["_ev_d"] = view["_create_d"]
        view = view.loc[view["_ev_d"].between(start_d, end_d, inclusive="both")].copy()
    ...
    try:
        import altair as alt
        plot_df = view.copy()
        plot_df["_bucket"] = _floor_period(plot_df["_ev_d"], gran)
        if split:
    ...
    try:
        ...
        st.download_button("Download invalid distribution (CSV)", data=csv_df.to_csv(index=False).encode("utf-8"),
                           file_name=f"Invalid_distribution_{mode}_{start_d}_to_{end_d}.csv", mime="text/csv",
                           key="dl_invalid_dist")
    except Exception as _e:
