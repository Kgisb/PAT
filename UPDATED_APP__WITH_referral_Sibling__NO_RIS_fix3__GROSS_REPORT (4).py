...
def _gross_trial_scheduled_count(df, create_col, fc_sched_col, cal_resched_col, mode, start, end):
    # New rule: if both dates exist for a deal -> consider only the Rescheduled date (count 1)
    _df = df.copy()
    ...
    if mode == "MTD":
        # Gate by Create Date window; then count 1 if either event exists; prefer reschedule if both
        m = (_df["_create_d"].between(start_d, end_d, inclusive="both"))
        sub = _df.loc[m]
        # count rows where reschedule exists OR (no reschedule but first schedule exists)
        cnt = int((sub["_cal_resched_d"].notna() | (sub["_cal_resched_d"].isna() & sub["_fc_sched_d"].notna())).sum())
        return cnt
    else:
        # Cohort: Count 1 per deal if relevant event date falls in window; prefer reschedule if present
        def _row_count(r):
            rd = r["_cal_resched_d"]
            sd = r["_fc_sched_d"]
            if pd.notna(rd):
...
def _row_mtd(r):
        # MTD: gate by Create Date window, then count events with de-dup when same-day
        if pd.isna(r["_create_d"]) or not (start_d <= r["_create_d"] <= end_d):
            return 0
        fs = pd.notna(r["_fc_sched_d"])
        rs = pd.notna(r["_cal_resched_d"])
        if fs and rs:
            return 1 if r["_fc_sched_d"] == r["_cal_resched_d"] else 2
        return int(fs) + int(rs)

    def _row_cohort(r):
        # Cohort: count by event dates that fall in window; if same-date both present -> count 1
        events = []
        if pd.notna(r["_fc_sched_d"]):
            events.append(r["_fc_sched_d"])
        if pd.notna(r["_cal_resched_d"]):
            events.append(r["_cal_resched_d"])
        if not events:
            return 0
        # if both exist and same date -> unique set collapses to one
        uniq = set(events)
        return sum(1 for d in uniq if start_d <= d <= end_d)

    if mode == "MTD":
        return int(_df.apply(_row_mtd, axis=1).sum())
    else:
        return int(_df.apply(_row_cohort, axis=1).sum())
