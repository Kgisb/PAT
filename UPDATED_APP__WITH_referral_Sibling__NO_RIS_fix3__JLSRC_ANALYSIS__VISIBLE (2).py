...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        ...
        "Funnel & Movement": ["JL Source-wise Analysis", "Funnel", "Lead Movement", "Stuck deals", "Deal Velocity", "Deal Decay", "Carry Forward", "Referral Pitched In", "Closed Lost Analysis", "Booking Analysis", "Trial Trend"],
...
# =====================
try:
    if "JL Source-wise Analysis" not in MASTER_SECTIONS.get("Funnel & Movement", []):
        MASTER_SECTIONS["Funnel & Movement"] = ["JL Source-wise Analysis"] + MASTER_SECTIONS.get("Funnel & Movement", [])
except Exception:
    pass
...
def _date_window_controls(_key_prefix="jlsrc"):
    import streamlit as st
    tz = "Asia/Kolkata"
    ...
    quick = st.selectbox("Date preset", ["Today","Yesterday","This Month","Last Month","Custom"], index=2, key=f"{_key_prefix}_quick")
    if quick == "Today":
        start, end = today, today
...
def _trial_sched_flag_row(fs_date, rs_date):
    fs = pd.notna(fs_date)
    rs = pd.notna(rs_date)
    if rs:
        return 1, rs_date
...
def _render_funnel_jl_source_analysis(df_f: pd.DataFrame):
    import streamlit as st
    st.subheader("JL Source-wise Analysis")
    df = df_f.copy()
    stage_col = _col_guess(df, ["Deal Stage","Stage","deal stage"])
    if stage_col is not None:
        df = df[~df[stage_col].astype(str).str.startswith("1.2 Invalid", na=False)].copy()
    ...
    src_col = _col_guess(df, ["JetLearn Deal Source","Deal Source","Source"])
    if src_col is None or create_col is None:
        st.error("Required columns missing."); return
    df["_create_d"] = _parse_date_series(df[create_col])
    df["_pay_d"] = _parse_date_series(df[pay_col]) if pay_col is not None else pd.NaT
    df["_fs_d"] = _parse_date_series(df[fc_sched_col]) if fc_sched_col is not None else pd.NaT
    df["_rs_d"] = _parse_date_series(df[cal_resched_col]) if cal_resched_col is not None else pd.NaT
    df["_done_d"] = _parse_date_series(df[cal_done_col]) if cal_done_col is not None else pd.NaT
    sched_marks = df.apply(lambda r: _trial_sched_flag_row(r["_fs_d"], r["_rs_d"]), axis=1, result_type="expand")
    df["_sched_flag"] = sched_marks[0].astype(int) if isinstance(sched_marks, pd.DataFrame) else 0
    df["_sched_d"] = sched_marks[1] if isinstance(sched_marks, pd.DataFrame) else pd.NaT
    mode, start, end = _date_window_controls("jlsrc")
    start_d = pd.to_datetime(start).date(); end_d = pd.to_datetime(end).date()
    if mode == "MTD":
        base = df["_create_d"].between(start_d, end_d, inclusive="both")
        dfv = df.loc[base].copy()
        deals_mask = pd.Series(True, index=dfv.index)
        enrl_mask = dfv["_pay_d"].notna()
        sched_mask = dfv["_sched_flag"] == 1
        done_mask = dfv["_done_d"].notna()
    else:
        dfv = df.copy()
        deals_mask = dfv["_create_d"].between(start_d, end_d, inclusive="both")
        enrl_mask = dfv["_pay_d"].between(start_d, end_d, inclusive="both")
        sched_mask = dfv["_sched_d"].between(start_d, end_d, inclusive="both")
        done_mask = dfv["_done_d"].between(start_d, end_d, inclusive="both")

    g = dfv.groupby(src_col, dropna=False)
    deals = g.apply(lambda x: int((x["_create_d"].between(start_d, end_d, inclusive="both")).sum()) if mode=="Cohort" else len(x)).rename("Deals")
    enrl = g.apply(lambda x: int((x["_pay_d"].between(start_d, end_d, inclusive="both")).sum()) if mode=="Cohort" else int(x["_pay_d"].notna().sum())).rename("Enrollments")
    ts = g.apply(lambda x: int((x["_sched_d"].between(start_d, end_d, inclusive="both")).sum()) if mode=="Cohort" else int((x["_sched_flag"]==1).sum())).rename("Trial Scheduled")
    td = g.apply(lambda x: int((x["_done_d"].between(start_d, end_d, inclusive="both")).sum()) if mode=="Cohort" else int(x["_done_d"].notna().sum())).rename("Trial Done")
    table = pd.concat([deals, enrl, ts, td], axis=1).fillna(0).astype(int)
    # Per-source percentage columns
    def _pct(a, b):
        return 0.0 if (b is None or b == 0) else round(100.0 * a / b, 1)
    # Existing: Lead→Enroll %
    table["Lead→Enroll %"] = [_pct(e, d) for e, d in zip(table["Enrollments"], table["Deals"])]
    # New: 3 additional % columns
    ...
    # --- Overall toggle (adds an Overall row and shows % KPI boxes) ---
    show_overall = st.toggle("Show Overall", value=True, key="jlsrc_overall_toggle")
    ...
    if show_overall:
        totals = table[["Deals","Enrollments","Trial Scheduled","Trial Done"]].sum()
        overall_pct_ts_vs_deals = _pct(totals["Trial Scheduled"], totals["Deals"])
        overall_pct_td_vs_ts = _pct(totals["Trial Done"], totals["Trial Scheduled"])
        overall_pct_enr_vs_td = _pct(totals["Enrollments"], totals["Trial Done"])
        overall_pct_enr_vs_deals = _pct(totals["Enrollments"], totals["Deals"])

        # Append Overall row at the bottom
        overall_row = {
            "Deals": int(totals["Deals"]),
            "Enrollments": int(totals["Enrollments"]),
            "Trial Scheduled": int(totals["Trial Scheduled"]),
            "Trial Done": int(totals["Trial Done"]),
            "Lead→Enroll %": _pct(int(totals["Enrollments"]), int(totals["Deals"]))
        }
        # Use a name unlikely to collide with a real source label
        table.loc["__OVERALL__"] = overall_row

        # Show four KPI % boxes
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("% Trial Scheduled vs Deals", f"{overall_pct_ts_vs_deals:.1f}%")
        c2.metric("% Trial Done vs Trial Scheduled", f"{overall_pct_td_vs_ts:.1f}%")
        c3.metric("% Enrolments vs Trial Done", f"{overall_pct_enr_vs_td:.1f}%")
        c4.metric("% Enrolments vs Deals", f"{overall_pct_enr_vs_deals:.1f}%")

    def _pct(a, b): return 0.0 if (b is None or b == 0) else round(100.0 * a / b, 1)
    table["Lead→Enroll %"] = [_pct(e, d) for e, d in zip(table["Enrollments"], table["Deals"])]
    st.caption(f"Mode: {mode} • Window: {start_d} → {end_d}")
    st.dataframe(table, use_container_width=True)
    try:
        st.download_button("Download JL Source-wise table (CSV)",
            data=table.reset_index().to_csv(index=False).encode("utf-8"),
            file_name=f"JL_Source_wise_{mode}_{start_d}_to_{end_d}.csv",
            mime="text/csv", key="dl_jlsrc_table")
    except Exception as _e:
        st.caption(f"(Download unavailable: {_e})")

# Wire into Funnel & Movement nav if available
try:
