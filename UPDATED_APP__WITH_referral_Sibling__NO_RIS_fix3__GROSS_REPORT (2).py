...
def _gross_trial_done_count(df, create_col, cal_done_col, pre_sched_col, mode, start, end):
    ...
    _df["_pre_d"] = _parse_date_series(_df[pre_sched_col]) if pre_sched_col else pd.NaT
    start_d = pd.to_datetime(start).date()
    end_d = pd.to_datetime(end).date()
    ...
    if mode == "MTD":
        ...
        sub = _df.loc[m]
        return int(sub["_done_d"].notna().sum() + sub["_pre_d"].notna().sum())
    else:
        c1 = _df["_done_d"].between(start_d, end_d, inclusive="both")
        c2 = _df["_pre_d"].between(start_d, end_d, inclusive="both")
