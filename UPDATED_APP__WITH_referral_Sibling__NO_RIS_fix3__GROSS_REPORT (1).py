...
def _render_marketing_gross_report(df_f: pd.DataFrame):
    ...
    pay_col = _col_guess(df, ["Payment Received Date","Enrollment Date"])
    ...
    cal_done_col = _col_guess(df, ["First Calibration Date","Calibration Done Date"])
    pre_sched_col = _col_guess(df, ["Calibration Pre-scheduled Date","Pre-scheduled Calibration Date"])
