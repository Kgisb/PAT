...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        ...
        "Funnel & Movement": ["Funnel", "Lead Movement", "Stuck deals", "Deal Velocity", "Deal Decay", "Carry Forward", "Referral Pitched In", "Closed Lost Analysis", "Booking Analysis"],
        ...
        "Marketing": ["Gross report", "Referrals","HubSpot Deal Score tracker","Marketing Lead Performance & Requirement","Kids detail", "Deal Detail", "Sales Intern Funnel", "Master analysis", "Referral Tracking", "Talk Time", "referral_Sibling"],
    }
...
# =====================
# Gross report (Marketing)
# =====================

# Ensure the new pill appears in the Marketing section
try:
    if "Gross report" not in MASTER_SECTIONS.get("Marketing", []):
        MASTER_SECTIONS["Marketing"] = ["Gross report"] + MASTER_SECTIONS.get("Marketing", [])
except Exception:
    pass

def _col_guess(_df, candidates):
...
def _parse_date_series(s):
    # Normalize to datetime (date only), tolerant to DD-MM-YYYY / YYYY-MM-DD
    dt = pd.to_datetime(s, errors="coerce", dayfirst=True, utc=False)
    return dt.dt.date

def _date_window_controls(_key_prefix="gross"):
    import streamlit as st
    import datetime as dt
    tz = "Asia/Kolkata"
    today = pd.Timestamp.now(tz=tz).date()
    first_of_month = today.replace(day=1)
    last_month_last_day = (first_of_month - pd.Timedelta(days=1))
    last_month_first_day = last_month_last_day.replace(day=1)

    mode = st.radio("Mode", ["MTD", "Cohort"], horizontal=True, key=f"{_key_prefix}_mode")
    quick = st.selectbox("Quick range", ["Today", "Yesterday", "This Month (MTD)", "Last Month", "Custom"],
                         index=2, key=f"{_key_prefix}_quick")
    if quick == "Today":
        start, end = today, today
        ...
        start, end = last_month_first_day, last_month_last_day
    else:
        start = st.date_input("Start", value=first_of_month, key=f"{_key_prefix}_start")
        end = st.date_input("End", value=today, key=f"{_key_prefix}_end")
        if isinstance(start, (list, tuple)):
            start = start[0]
...
def _gross_trial_scheduled_count(df, create_col, fc_sched_col, cal_resched_col, mode, start, end):
    # New rule: if both exist for a deal -> consider only the Rescheduled date (count 1)
    _df = df.copy()
    _df["_create_d"] = _parse_date_series(_df[create_col]) if create_col else pd.NaT
    _df["_fc_sched_d"] = _parse_date_series(_df[fc_sched_col]) if fc_sched_col else pd.NaT
    _df["_cal_resched_d"] = _parse_date_series(_df[cal_resched_col]) if cal_resched_col else pd.NaT

    start_d = pd.to_datetime(start).date()
    end_d = pd.to_datetime(end).date()

    if mode == "MTD":
        m = _df["_create_d"].between(start_d, end_d, inclusive="both")
        sub = _df.loc[m]
        return int((sub["_cal_resched_d"].notna() | (sub["_cal_resched_d"].isna() & sub["_fc_sched_d"].notna())).sum())
    else:
        def _row_count(r):
            rd = r["_cal_resched_d"]
            sd = r["_fc_sched_d"]
            if pd.notna(rd):
                return int(start_d <= rd <= end_d)
            elif pd.notna(sd):
                return int(start_d <= sd <= end_d)
            else:
                return 0
        return int(_df.apply(_row_count, axis=1).sum())

def _gross_trial_done_count(df, create_col, cal_done_col, pre_sched_col, mode, start, end):
    # Trial Done should count ONLY from "Calibration Done Date"
    _df = df.copy()
    _df["_create_d"] = _parse_date_series(_df[create_col]) if create_col else pd.NaT
    _df["_done_d"] = _parse_date_series(_df[cal_done_col]) if cal_done_col else pd.NaT
    start_d = pd.to_datetime(start).date()
    end_d = pd.to_datetime(end).date()

    if mode == "MTD":
        # Count done among deals created in window
        m = _df["_create_d"].between(start_d, end_d, inclusive="both")
        sub = _df.loc[m]
        return int(sub["_done_d"].between(start_d, end_d, inclusive="both").sum()) if "_done_d" in sub else int(sub["_done_d"].notna().sum())
    else:
        # Cohort: done date falls in window
        return int(_df["_done_d"].between(start_d, end_d, inclusive="both").sum())


def _gross_enrollment_count(df, create_col, pay_col, mode, start, end):
    _df = df.copy()
    _df["_create_d"] = _parse_date_series(_df[create_col]) if create_col else pd.NaT
    _df["_pay_d"] = _parse_date_series(_df[pay_col]) if pay_col else pd.NaT
    start_d = pd.to_datetime(start).date()
    end_d = pd.to_datetime(end).date()
    if mode == "MTD":
        m = _df["_create_d"].between(start_d, end_d, inclusive="both")
        return int(_df.loc[m, "_pay_d"].notna().sum())
    else:
        return int(_df["_pay_d"].between(start_d, end_d, inclusive="both").sum())

def _gross_deals_count(df, create_col, mode, start, end):
    _df = df.copy()
    _df["_create_d"] = _parse_date_series(_df[create_col]) if create_col else pd.NaT
    start_d = pd.to_datetime(start).date()
    end_d = pd.to_datetime(end).date()
    return int(_df["_create_d"].between(start_d, end_d, inclusive="both").sum())

def _render_marketing_gross_report(df_f: pd.DataFrame):
    import streamlit as st
    st.subheader("Gross report")

    df = df_f.copy()

    # Exclude 1.2 Invalid deals if Deal Stage exists
    stage_col = _col_guess(df, ["Deal Stage","Stage","deal stage"])
    if stage_col is not None:
        _mask_valid = ~df[stage_col].astype(str).str.startswith("1.2 Invalid", na=False)
        df = df.loc[_mask_valid].copy()

    # Resolve key columns
    create_col = _col_guess(df, ["Create Date","Deal Create Date","Created Date"])
    pay_col = _col_guess(df, ["Payment Received Date","Payment Received Date ","Enrollment Date"])
    fc_sched_col = _col_guess(df, ["First Calibration Scheduled Date","First Calibration Booking Date","Calibration Scheduled Date"])
    cal_resched_col = _col_guess(df, ["Calibration Rescheduled Date","Trial Reschedule 1","Trial Reschedule 2"])
    cal_done_col = _col_guess(df, ["Calibration Done Date"])
    pre_sched_col = _col_guess(df, ["Calibration Pre-scheduled Date","Pre-scheduled Calibration Date","[Trigger] - Calibration Booking Date"])

    # Controls
    mode, start, end = _date_window_controls("gross")

    # KPI calculations
    try:
        deals = _gross_deals_count(df, create_col, mode, start, end)
    except Exception:
        deals = 0
    try:
        enroll = _gross_enrollment_count(df, create_col, pay_col, mode, start, end)
    except Exception:
        enroll = 0
    try:
        trial_sched = _gross_trial_scheduled_count(df, create_col, fc_sched_col, cal_resched_col, mode, start, end)
    except Exception:
        trial_sched = 0
    try:
        trial_done = _gross_trial_done_count(df, create_col, cal_done_col, pre_sched_col, mode, start, end)
    except Exception:
        trial_done = 0

    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Deals", f"{deals:,}")
    c2.metric("Enrollments", f"{enroll:,}")
    c3.metric("Trial Scheduled", f"{trial_sched:,}")
    c4.metric("Trial Done", f"{trial_done:,}")

    st.caption(f"Mode: {mode} • Window: {start} → {end}")
    # --------------------
    # Overall % view (optional)
    # --------------------
    view_kind = st.radio("View", ["Counts", "Overall %"], index=0, horizontal=True, key="gross_view_kind")
    if view_kind == "Overall %":
        def _pc(a, b):
            try:
                return 0.0 if (b is None or b == 0) else round(100.0 * a / b, 1)
            except Exception:
                return 0.0
        p1 = _pc(trial_sched, deals)           # % Trial Scheduled vs Deals
        p2 = _pc(trial_done, trial_sched)      # % Trial Done vs Trial Scheduled
        p3 = _pc(enroll, trial_done)           # % Enrolments vs Trial Done
        p4 = _pc(enroll, deals)                # % Enrolments vs Deals

        pc1, pc2, pc3, pc4 = st.columns(4)
        pc1.metric("% Trial Scheduled vs Deals", f"{p1:.1f}%")
        pc2.metric("% Trial Done vs Trial Scheduled", f"{p2:.1f}%")
        pc3.metric("% Enrolments vs Trial Done", f"{p3:.1f}%")
        pc4.metric("% Enrolments vs Deals", f"{p4:.1f}%")

    # --------------------
    # Optional breakdowns
    # --------------------
    with st.expander("Breakdowns (optional): Academic Counsellor-wise & Date-wise"):
        # Resolve AC column
        ac_col = _col_guess(df, [
        ...
        # Helper: event dates per metric for grouping
        df["_create_d"] = _parse_date_series(df[create_col]) if create_col else pd.NaT
        df["_pay_d"] = _parse_date_series(df[pay_col]) if pay_col else pd.NaT
        df["_sched_event_d"] = pd.NaT
        if cal_resched_col:
            df["_sched_event_d"] = _parse_date_series(df[cal_resched_col])
        if fc_sched_col:
            # fill only where resched missing
            _fill = _parse_date_series(df[fc_sched_col])
            df.loc[df["_sched_event_d"].isna(), "_sched_event_d"] = _fill
        df["_done_d"] = _parse_date_series(df[cal_done_col]) if cal_done_col else pd.NaT

        # Window masks per mode
        start_d = pd.to_datetime(start).date()
        end_d = pd.to_datetime(end).date()
        if mode == "MTD":
            mask_base = df["_create_d"].between(start_d, end_d, inclusive="both")
            df_view = df.loc[mask_base].copy()
            # Metric filters
            m_deals = pd.Series(True, index=df_view.index)
            m_enrl = df_view["_pay_d"].notna()
            m_sched = df_view["_sched_event_d"].notna()
            m_done = df_view["_done_d"].notna() if "_done_d" in df_view else pd.Series(False, index=df_view.index)
        else:
            # Cohort: each metric by its own event date in window
            df_view = df.copy()
            m_deals = df_view["_create_d"].between(start_d, end_d, inclusive="both")  # useful if user wants create-date cohort
            m_enrl = df_view["_pay_d"].between(start_d, end_d, inclusive="both")
            m_sched = df_view["_sched_event_d"].between(start_d, end_d, inclusive="both")
            m_done = df_view["_done_d"].between(start_d, end_d, inclusive="both")

        # --- Academic Counsellor-wise table
        if ac_col is not None:
            import pandas as _pd

            ac_df = _pd.DataFrame({
                "Deals": df_view.loc[m_deals, ac_col]
                    .value_counts(dropna=False).rename("Deals"),
                "Enrollments": df_view.loc[m_enrl, ac_col]
                    .value_counts(dropna=False).rename("Enrollments"),
                "Trial Scheduled": df_view.loc[m_sched, ac_col]
                    .value_counts(dropna=False).rename("Trial Scheduled"),
                "Trial Done": df_view.loc[m_done, ac_col]
                    .value_counts(dropna=False).rename("Trial Done"),
            })
            ac_df = ac_df.fillna(0).astype(int)

            # ---- Derived percentage columns ----
            def _pct(num, den):
                return (100.0 * num / den) if den not in (0, None) else 0.0

            ac_df["% Trial Scheduled vs Deals"] = [_pct(ts, dl) for ts, dl in zip(ac_df["Trial Scheduled"], ac_df["Deals"])]
            ac_df["% Trial Done vs Trial Scheduled"] = [
                _pct(td, ts) for td, ts in zip(ac_df["Trial Done"], ac_df["Trial Scheduled"])
            ]
            ac_df["% Enrolments vs Trial Done"] = [
                _pct(enr, td) for enr, td in zip(ac_df["Enrollments"], ac_df["Trial Done"])
            ]
            ac_df["% Enrolments vs Deals"] = [
                _pct(enr, dl) for enr, dl in zip(ac_df["Enrollments"], ac_df["Deals"])
            ]

            # Round to 1 decimal for readability
            for c in ["% Trial Scheduled vs Deals", "% Trial Done vs Trial Scheduled", "% Enrolments vs Trial Done", "% Enrolments vs Deals"]:
                ac_df[c] = ac_df[c].round(1)

            # Sort with primary = Deals desc, secondary = Enrolments desc
            ac_df = ac_df.sort_values(by=["Deals","Enrollments"], ascending=[False, False])

            st.markdown("**Academic Counsellor-wise**")
            st.dataframe(ac_df)
            # ---- Combined chart (pick any two columns) ----
            metric_choices = [c for c in ac_df.columns if (ac_df[c].dtype.kind in "iuf")]
            # Exclude purely derived % metrics if user wants only counts (comment out to include all)
            # metric_choices_counts = [c for c in metric_choices if not c.startswith("%")]
            sel = st.multiselect(
                "Select any two metrics for combined chart",
                options=metric_choices,
                default=[m for m in ["Deals", "Enrollments"] if m in metric_choices][:2],
                key="gross_ac_combo_sel",
                max_selections=2
            )
            top_n = st.number_input("Top N rows (by Deals)", min_value=1, value=15, step=1, key="gross_ac_combo_topn")
            ac_plot_df = ac_df.copy()
            # Keep top N by Deals when present
            if "Deals" in ac_plot_df.columns:
                ac_plot_df = ac_plot_df.sort_values(by="Deals", ascending=False).head(int(top_n))
            ac_plot_df = ac_plot_df.reset_index()
            # Rename the first column to a stable label regardless of original index name
            first_col = ac_plot_df.columns[0]
            if first_col != "Academic Counsellor":
                ac_plot_df = ac_plot_df.rename(columns={first_col: "Academic Counsellor"})
            if len(sel) == 2:
                try:
                    import pandas as _pd
                    import altair as alt
                    melt_df = ac_plot_df.melt(id_vars=["Academic Counsellor"], value_vars=sel, var_name="Metric", value_name="Value")
                    chart = alt.Chart(melt_df).mark_bar().encode(
                        x=alt.X("Academic Counsellor:N", sort="-y"),
                        y=alt.Y("Value:Q"),
                        color="Metric:N",
                        column=alt.Column("Metric:N", header=alt.Header(title=None)),
                        tooltip=["Academic Counsellor", "Metric", "Value"]
                    ).properties(height=300)
                    st.altair_chart(chart, use_container_width=True)
                except Exception as _e:
                    st.caption(f"(Could not render combined chart: {_e})")
            else:
                st.info("Pick exactly two metrics to render the combined chart.")



        # --- Date-wise table(s)
        gran = st.selectbox("Date granularity for date-wise tables",
                            ["Day", "Week (Mon start)", "Month"],
                            index=0, key="gross_date_gran")
        def _floor_to_period(s, granularity):
            idx = pd.to_datetime(s, errors="coerce")
            if granularity.startswith("Day"):
                return idx.dt.to_period("D").dt.to_timestamp()
            if granularity.startswith("Week"):
                # normalize to week starting Monday
                # pandas week period W-MON
                return idx.dt.to_period("W-MON").dt.start_time
            return idx.dt.to_period("M").dt.to_timestamp()

        import pandas as _pd
        st.markdown("**Date-wise**")
        # Deals (use create date)
        deals_series = df_view.loc[m_deals, "_create_d"]
        deals_group = deals_series.dropna()
        if not deals_group.empty:
            _dg = _pd.Series(1, index=_floor_to_period(deals_group, gran)).groupby(level=0).sum().rename("Deals")
            st.write("Deals")
            st.dataframe(_dg.to_frame())

        # Enrollments (use pay date in cohort; create-based in MTD already filtered)
        enrl_series = df_view.loc[m_enrl, "_pay_d"] if mode == "Cohort" else df_view.loc[m_enrl, "_create_d"]
        enrl_group = enrl_series.dropna()
        if not enrl_group.empty:
            _eg = _pd.Series(1, index=_floor_to_period(enrl_group, gran)).groupby(level=0).sum().rename("Enrollments")
            st.write("Enrollments")
            st.dataframe(_eg.to_frame())

        # Trial Scheduled (use sched event date in both modes for date-wise view)
        sched_series = df_view.loc[m_sched, "_sched_event_d"]
        sched_group = sched_series.dropna()
        if not sched_group.empty:
            _sg = _pd.Series(1, index=_floor_to_period(sched_group, gran)).groupby(level=0).sum().rename("Trial Scheduled")
            st.write("Trial Scheduled")
            st.dataframe(_sg.to_frame())

        # Trial Done (use done date in both modes for date-wise view)
        done_series = df_view.loc[m_done, "_done_d"]
        done_group = done_series.dropna()
        if not done_group.empty:
            _tg = _pd.Series(1, index=_floor_to_period(done_group, gran)).groupby(level=0).sum().rename("Trial Done")
            st.write("Trial Done")
            st.dataframe(_tg.to_frame())

    with st.expander("Column mapping (auto-detected)"):
        st.write({
            "Create Date": create_col,
            "Payment Received Date": pay_col,
            "First Calibration Scheduled Date": fc_sched_col,
            "Calibration Rescheduled Date": cal_resched_col,
            "First Calibration Done Date": cal_done_col,
            "Calibration Pre-scheduled Date": pre_sched_col,
        })

# ---- Dispatcher for Marketing ▸ Gross report
try:
    if st.session_state.get('nav_master') == "Marketing" and st.session_state.get('nav_sub') == "Gross report":
        _render_marketing_gross_report(df_f)
except Exception as _e:
    import streamlit as _st
    _st.error(f"Gross report error: {_e}")
