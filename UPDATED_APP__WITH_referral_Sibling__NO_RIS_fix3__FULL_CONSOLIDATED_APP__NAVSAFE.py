...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        ...
        "Marketing": ["Referrals","HubSpot Deal Score tracker","Marketing Lead Performance & Requirement","Kids detail", "Deal Detail", "Sales Intern Funnel", "Master analysis", "Referral Tracking", "Talk Time", "referral_Sibling"],
    }
...
# =====================
# Funnel & Movement ▸ JL Source-wise Analysis (restored)
# =====================
try:
    if "Funnel & Movement" not in MASTER_SECTIONS:
        MASTER_SECTIONS["Funnel & Movement"] = []
    if "JL Source-wise Analysis" not in MASTER_SECTIONS["Funnel & Movement"]:
        MASTER_SECTIONS["Funnel & Movement"] = ["JL Source-wise Analysis"] + MASTER_SECTIONS["Funnel & Movement"]
except Exception:
    pass

def _jls_col_guess(_df, candidates):
    cols = {str(c).lower().strip(): c for c in _df.columns}
    for c in candidates:
    ...
    for c in candidates:
        for col in _df.columns:
            if str(c).lower().strip() in str(col).lower(): return col
    return None

def _jls_parse_date_series(s):
    import pandas as pd
    dt = pd.to_datetime(s, errors="coerce", dayfirst=True, utc=False)
    try: return dt.dt.date
    except Exception: return pd.NaT

def _jls_date_controls(key="jlsrc"):
    import streamlit as st, pandas as pd
    tz = "Asia/Kolkata"
    today = pd.Timestamp.now(tz=tz).date()
    first = today.replace(day=1)
    last_month_last = first - pd.Timedelta(days=1)
    last_month_first = last_month_last.replace(day=1)
    mode = st.radio("Mode", ["MTD","Cohort"], horizontal=True, key=f"{key}_mode")
    preset = st.selectbox("Date preset", ["Today","Yesterday","This Month","Last Month","Custom"], index=2, key=f"{key}_preset")
    if preset == "Today":
        start, end = today, today
    elif preset == "Yesterday":
        y = today - pd.Timedelta(days=1); start, end = y, y
    elif preset == "This Month":
        start, end = first, today
    elif preset == "Last Month":
        start, end = last_month_first, last_month_last
    else:
        c1, c2 = st.columns(2)
        with c1: start = st.date_input("Start", value=first, key=f"{key}_start")
        with c2: end = st.date_input("End", value=today, key=f"{key}_end")
        if isinstance(start, (list, tuple)): start = start[0]
        if isinstance(end, (list, tuple)): end = end[0]
    return mode, start, end

def _render_funnel_jl_source_analysis(df_f):
    import streamlit as st, pandas as pd
    df = df_f.copy()
    st.subheader("JL Source-wise Analysis")

    # Exclude invalid
    stage_col = _jls_col_guess(df, ["Deal Stage","Stage"])
    if stage_col is not None:
        df = df[~df[stage_col].astype(str).str.contains("invalid", case=False, na=False)].copy()

    create_col = _jls_col_guess(df, ["Create Date","Deal Create Date","Created Date"])
    pay_col    = _jls_col_guess(df, ["Payment Received Date","Payment Received Date ","Enrollment Date"])
    fs         = _jls_col_guess(df, ["First Calibration Scheduled Date","First Calibration Booking Date","Calibration Scheduled Date"])
    rs         = _jls_col_guess(df, ["Calibration Rescheduled Date","Trial Reschedule 1","Trial Reschedule 2"])
    done       = _jls_col_guess(df, ["Calibration Done Date"])
    src_col    = _jls_col_guess(df, ["JetLearn Deal Source","Deal Source","Source"])
    if src_col is None or create_col is None:
        st.error("Required columns not found: Deal Source / Create Date"); return

    df["_create_d"] = _jls_parse_date_series(df[create_col])
    df["_pay_d"]    = _jls_parse_date_series(df[pay_col]) if pay_col is not None else pd.NaT
    df["_fs_d"]     = _jls_parse_date_series(df[fs]) if fs is not None else pd.NaT
    df["_rs_d"]     = _jls_parse_date_series(df[rs]) if rs is not None else pd.NaT
    df["_done_d"]   = _jls_parse_date_series(df[done]) if done is not None else pd.NaT
    df["_sched_d"]  = df["_rs_d"].where(df["_rs_d"].notna(), df["_fs_d"])  # prefer resched

    mode, start, end = _jls_date_controls("jlsrc")
    start_d = pd.to_datetime(start).date(); end_d = pd.to_datetime(end).date()

    if mode == "MTD":
        view = df[df["_create_d"].between(start_d, end_d, inclusive="both")].copy()
        deals = view.groupby(src_col).size().rename("Deals")
        enrl  = view.groupby(src_col)["_pay_d"].apply(lambda s: int(s.notna().sum())).rename("Enrollments")
        ts    = view.groupby(src_col)["_sched_d"].apply(lambda s: int(s.notna().sum())).rename("Trial Scheduled")
        td    = view.groupby(src_col)["_done_d"].apply(lambda s: int(s.notna().sum())).rename("Trial Done")
    else:
        view = df.copy()
        deals = view.groupby(src_col)["_create_d"].apply(lambda s: int(s.between(start_d, end_d, inclusive="both").sum())).rename("Deals")
        enrl  = view.groupby(src_col)["_pay_d"].apply(   lambda s: int(s.between(start_d, end_d, inclusive="both").sum())).rename("Enrollments")
        ts    = view.groupby(src_col)["_sched_d"].apply( lambda s: int(s.between(start_d, end_d, inclusive="both").sum())).rename("Trial Scheduled")
        td    = view.groupby(src_col)["_done_d"].apply(  lambda s: int(s.between(start_d, end_d, inclusive="both").sum())).rename("Trial Done")

    table = pd.concat([deals, enrl, ts, td], axis=1).fillna(0).astype(int)

    def _pct(a, b): 
        return 0.0 if (b is None or b == 0) else round(100.0 * a / b, 1)
    table["Lead→Enroll %"]               = [_pct(e, d) for e, d in zip(table["Enrollments"], table["Deals"])]
    table["% Trial Scheduled vs Deals"]  = [_pct(ts, dl) for ts, dl in zip(table["Trial Scheduled"], table["Deals"])]
    table["% Trial Done vs Trial Scheduled"] = [_pct(td, ts) for td, ts in zip(table["Trial Done"], table["Trial Scheduled"])]
    table["% Enrolments vs Trial Done"]  = [_pct(en, td) for en, td in zip(table["Enrollments"], table["Trial Done"])]

    st.caption(f"Mode: {mode} • Window: {start_d} → {end_d}")
    st.dataframe(table, use_container_width=True)

# Dispatcher for JL Source-wise
try:
    if st.session_state.get('nav_master') == "Funnel & Movement" and st.session_state.get('nav_sub') == "JL Source-wise Analysis":
        _render_funnel_jl_source_analysis(df_f)
except Exception as _e:
    import streamlit as _st
    _st.error(f"JL Source-wise Analysis error: {_e}")



# =====================
# Marketing ▸ Invalid deal trend (restored)
# =====================
try:
    if "Marketing" not in MASTER_SECTIONS:
        MASTER_SECTIONS["Marketing"] = []
    if "Invalid deal trend" not in MASTER_SECTIONS["Marketing"]:
        MASTER_SECTIONS["Marketing"] = ["Invalid deal trend"] + MASTER_SECTIONS["Marketing"]
except Exception:
    pass

def _inv_col_guess(_df, candidates):
    cols = {str(c).lower().strip(): c for c in _df.columns}
    for c in candidates:
        if c in _df.columns: return c
    for c in candidates:
...
def _inv_parse_date_series(s):
    import pandas as pd
    dt = pd.to_datetime(s, errors="coerce", dayfirst=True, utc=False)
    try: return dt.dt.date
    except Exception: return pd.NaT

def _inv_date_controls(key="invdeal"):
    import streamlit as st, pandas as pd
    tz = "Asia/Kolkata"
    today = pd.Timestamp.now(tz=tz).date()
    first = today.replace(day=1)
    last_month_last = first - pd.Timedelta(days=1)
    last_month_first = last_month_last.replace(day=1)
    mode = st.radio("Mode", ["MTD","Cohort"], index=0, horizontal=True, key=f"{key}_mode")
    preset = st.selectbox("Date preset", ["Today","Yesterday","This Month","Last Month","Custom"], index=2, key=f"{key}_preset")
    if preset == "Today":
        start, end = today, today
    elif preset == "Yesterday":
        ...
        if isinstance(start, (list, tuple)): start = start[0]
        if isinstance(end, (list, tuple)): end = end[0]
    gran = st.selectbox("Granularity", ["Day","Week (Mon)","Month"], index=2, key=f"{key}_gran")
    split = st.checkbox("Split by Invalid Stage Type", value=True, key=f"{key}_split")
    return mode, start, end, gran, split

def _floor_period_inv(series, gran):
    import pandas as pd
    idx = pd.to_datetime(series, errors="coerce")
    if gran.startswith("Day"): return idx.dt.to_period("D").dt.to_timestamp()
    if "Week" in gran: return idx.dt.to_period("W-MON").dt.start_time
    return idx.dt.to_period("M").dt.to_timestamp()

def _render_marketing_invalid_deal_trend(df_f):
    import streamlit as st, pandas as pd
    st.subheader("Invalid deal trend")
    df = df_f.copy()
    stage_col  = _inv_col_guess(df, ["Deal Stage","Stage"])
    create_col = _inv_col_guess(df, ["Create Date","Deal Create Date","Created Date"])
    if stage_col is None or create_col is None:
        st.error("Required columns not found: Deal Stage / Create Date"); return
    df["_is_invalid"] = df[stage_col].astype(str).str.contains("invalid", case=False, na=False)
    if not df["_is_invalid"].any():
        st.info("No rows with 'invalid' found in Deal Stage."); return
    df["_create_d"] = _inv_parse_date_series(df[create_col])
    inv = df.loc[df["_is_invalid"]].copy()
    inv["_invalid_type"] = df[stage_col].astype(str)
    mode, start, end, gran, split = _inv_date_controls("invdeal")
    start_d = pd.to_datetime(start).date(); end_d = pd.to_datetime(end).date()
    if mode == "MTD":
        view = inv[inv["_create_d"].between(start_d, end_d, inclusive="both")].copy()
        view["_ev_d"] = view["_create_d"]
    else:
        view = inv.copy()
        view["_ev_d"] = view["_create_d"]
        view = view[view["_ev_d"].between(start_d, end_d, inclusive="both")].copy()
    dist = view.groupby("_invalid_type", dropna=False).size().rename("Count").sort_values(ascending=False)
    st.markdown("**Invalid distribution (by Deal Stage value containing 'invalid')**")
    st.dataframe(dist.to_frame(), use_container_width=True)
    try:
        import altair as alt
        plot_df = view.copy()
        plot_df["_bucket"] = _floor_period_inv(plot_df["_ev_d"], gran)
        if split:
            agg = plot_df.groupby(["_bucket","_invalid_type"]).size().reset_index(name="Count")
            chart = alt.Chart(agg).mark_line(point=True).encode(
                x=alt.X("_bucket:T", title="Date"),
                y=alt.Y("Count:Q"),
                color=alt.Color("_invalid_type:N", title="Invalid Type"),
                tooltip=["_bucket:T","_invalid_type:N","Count:Q"]
            ).properties(height=340)
        else:
            agg = plot_df.groupby(["_bucket"]).size().reset_index(name="Count")
            chart = alt.Chart(agg).mark_line(point=True).encode(
                x=alt.X("_bucket:T", title="Date"),
                y=alt.Y("Count:Q"),
                tooltip=["_bucket:T","Count:Q"]
            ).properties(height=340)
        st.markdown("**Trend**")
        st.altair_chart(chart, use_container_width=True)
    except Exception as _e:
        st.caption(f"(Could not render trend chart: {_e})")
    st.caption(f"Mode: {mode} • Window: {start_d} → {end_d} • Granularity: {gran}")
    try:
        csv_df = dist.reset_index().rename(columns={"_invalid_type":"Invalid Stage"})
        st.download_button("Download invalid distribution (CSV)",
                           data=csv_df.to_csv(index=False).encode("utf-8"),
                           file_name=f"Invalid_distribution_{mode}_{start_d}_to_{end_d}.csv",
                           mime="text/csv", key="dl_invalid_dist")
    except Exception as _e:
        st.caption(f"(CSV unavailable: {_e})")

# Dispatcher for Invalid deal trend
try:
    if st.session_state.get('nav_master') == "Marketing" and st.session_state.get('nav_sub') == "Invalid deal trend":
        _render_marketing_invalid_deal_trend(df_f)
except Exception as _e:
    import streamlit as _st
    _st.error(f"Invalid deal trend error: {_e}")



# =====================
# NAV SAFETY BOOTSTRAP (ensures pills are visible & selectable)
# =====================
try:
    import streamlit as st
    # Ensure MASTER_SECTIONS exists
    if 'MASTER_SECTIONS' not in globals():
        MASTER_SECTIONS = {}
    # Ensure sections exist
    MASTER_SECTIONS.setdefault("Funnel & Movement", [])
    MASTER_SECTIONS.setdefault("Marketing", [])

    # Ensure our pills are registered
    if "JL Source-wise Analysis" not in MASTER_SECTIONS["Funnel & Movement"]:
        MASTER_SECTIONS["Funnel & Movement"] = ["JL Source-wise Analysis"] + MASTER_SECTIONS["Funnel & Movement"]
    for pill in ["AC Performance", "Invalid deal trend"]:
        if pill not in MASTER_SECTIONS["Marketing"]:
            MASTER_SECTIONS["Marketing"] = [pill] + MASTER_SECTIONS["Marketing"]

    # Minimal sidebar helper in case host app doesn't show these pills in its own nav
    with st.sidebar.expander("Extra pills", expanded=False):
        choice = st.radio("Open", ["—",
                                   "Funnel & Movement ▸ JL Source-wise Analysis",
                                   "Marketing ▸ AC Performance",
                                   "Marketing ▸ Invalid deal trend"], index=0, key="nav_helper_choice")
        if choice == "Funnel & Movement ▸ JL Source-wise Analysis":
            st.session_state['nav_master'] = "Funnel & Movement"
            st.session_state['nav_sub'] = "JL Source-wise Analysis"
        elif choice == "Marketing ▸ AC Performance":
            st.session_state['nav_master'] = "Marketing"
            st.session_state['nav_sub'] = "AC Performance"
        elif choice == "Marketing ▸ Invalid deal trend":
            st.session_state['nav_master'] = "Marketing"
            st.session_state['nav_sub'] = "Invalid deal trend"

    # If user selected via helper but host nav doesn't auto render, call the renderers directly
    try:
        if st.session_state.get('nav_master') == "Funnel & Movement" and st.session_state.get('nav_sub') == "JL Source-wise Analysis":
            if 'df_f' in globals():
                _render_funnel_jl_source_analysis(df_f)
        if st.session_state.get('nav_master') == "Marketing" and st.session_state.get('nav_sub') == "AC Performance":
            if 'df_f' in globals():
                _render_marketing_ac_performance(df_f)
        if st.session_state.get('nav_master') == "Marketing" and st.session_state.get('nav_sub') == "Invalid deal trend":
            if 'df_f' in globals():
                _render_marketing_invalid_deal_trend(df_f)
    except Exception as _e:
        st.caption(f"(Nav helper rendering note: {_e})")
except Exception:
    pass
